       ...
       <ul class="myFabulous_ul1" v-show="tabIndex==1&&articleData.length>0">
         <li v-for="(item,index) in articleData" @click="goHomeDetail(item)">
              <div class="myFabulous_left">
                ...
                <div>
                  <img src="/static/images/喜欢数.svg" alt=""> {{item.laudedCount?(item.laudedCount/10000).toFixed(2)+'万':item.laudedCount}}
                  <img src="/static/images/评论数.svg"  alt="" style="margin-left: 0.3rem"> {{item.commentCount?(item.commentCount/10000).toFixed(2)+'万':item.commentCount}}</div>
              </div>
...
export default {
  ...
  created(){
    ...
    if(this.userInfo){
      ...
      customerLaudNoteListFavoredContent(this.userInfo.data.id,this.userInfo.data.access_token,this.p,this.s).then(res=>{//作品
  ...
  methods: {
    ...
    tabClick(i){//tab的下标
      ...
      if(i==1){ //文章
        ...
        customerLaudNoteLaudList(this.userInfo.data.id,this.userInfo.data.access_token,this.p,this.s).then(res=>{
          if(res.status == true){
