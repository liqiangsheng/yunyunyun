...
<script>
  import { sysUserOperationUpdateUserPassword,telCode} from '../../assets/js/promiseHttp';
  ...
  methods:{
    ...
    loginBnt(){ // 完成
      setTimeout(()=>{
        ...
        sysUserOperationUpdateUserPassword(that.telValue,that.psdValue,that.password,this.userInfo.data.access_token).then(res=>{
          console.log(res,"修改密码")
          if(res.data.status == true){
            Toast('修改密码成功，下次可以使用新密码登陆');
            setTimeout(()=>{
