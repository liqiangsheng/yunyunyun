     ...
     <div class="multiActivityItem">
       <div class='multiActivityBoxHeader'>
         ...
         <div>
           ...
           <div  style='color:#21CB61'  v-if='listData.payPrice<=0'>免费</div>
       ...
       <div class="activityGoodsVoList" v-if="listData.activityGoodsGroupVoList&&listData.activityGoodsGroupVoList.length>0">
         ...
         <div class="activityGoodsVoListItem"  v-for="(item,index) in listData.activityGoodsGroupVoList" @click="bgItemTap(index,'two')" :class="{active:item.bgItem == true}">
           ...
           <div class="originalPrice">
                 ...
                 <div class='multiActivityBoxState1' v-else-if='item.remainingState=="未开始"&&item.bgItem==false'>
                   {{item.remainingState}}
                 </div>
                <div class='multiActivityBoxState1' v-else-if='item.remainingState=="已结束"&&item.bgItem==false'>
                   {{item.remainingState}}
                 </div>
                 <div class='multiActivityBoxState'  v-else-if='item.remainingState=="抢票中"&&item.bgItem==false'>
                   {{item.remainingState}}
            ...
            if(res.status == true){
               if( res.data.activityGoodsVoList&&res.data.activityGoodsVoList.length>0){
                 res.data.activityGoodsVoList.forEach((item,index)=>{
                   ...
                   if (nowTime < res.data.startTime){  //活动的开始时间
                     res.data.activityGoodsVoList[index].remainingState = "未开始"
                     res.data.activityGoodsVoList[index].bntBj = false;
                   } else if (nowTime >= res.data.endTime){  //活动的开始时间
                     res.data.activityGoodsVoList[index].remainingState = "已结束"
                     res.data.activityGoodsVoList[index].bntBj = false;
                   }else if (res.data.activityGoodsVoList[index].remaining<=0||res.data.activityGoodsVoList[index].remaining==null){
              ...
              if( res.data.activityGoodsGroupVoList&&res.data.activityGoodsGroupVoList.length>0){
                res.data.activityGoodsGroupVoList.forEach((item,index)=>{
                  ...
                  if (nowTime < res.data.startTime){  //活动的开始时间
                    res.data.activityGoodsGroupVoList[index].remainingState = "未开始"
                    res.data.activityGoodsGroupVoList[index].bntBj = false;
                  }else if (nowTime >= res.data.endTime){  //活动的开始时间
                    res.data.activityGoodsGroupVoList[index].remainingState = "已结束"
                    res.data.activityGoodsGroupVoList[index].bntBj = false;
                  }else if (res.data.activityGoodsGroupVoList[index].remaining<=0||res.data.activityGoodsGroupVoList[index].remaining==null){
  ...
  methods:{
    bgItemTap(i,v){ //点击每个票
      ...
      if (v == "one"){ //点击的单票
        ...
        if ( that.listData.activityGoodsVoList[bgIndex].remainingState == "未开始"){
          Toast("该活动未开始")
          return;
        }
        if ( that.listData.activityGoodsVoList[bgIndex].remainingState == "已结束"){
          Toast("该门票已售已结束")
          return;
        }
        ...
        if ( that.listData.activityGoodsGroupVoList[bgIndex].remainingState == "未开始"){
          Toast("该活动未开始")
          return;

        }
        if ( that.listData.activityGoodsGroupVoList[bgIndex].remainingState == "已结束"){
          Toast("该门票已售已结束")
          return;
