 ...
 */
  ...
  pageData(){ // 页面数据
    ...
    var that = this;
    wx.request({
      url: API.apiDomain + '/apis/activity/' + API. activityEdition+'/activityInfo/list',
      header: {
  ...
  dictionaries() {//字典缓存本地
    wx.request({
      url: API.apiDomain + '/apis/system/' + API.systemEdition+'/init/loadDicTree',
      success(res) {
        if (res.data.status == true) {
  ...
  goToPicture(e){ // 去火直播webview
    console.log(e.currentTarget.dataset.item)
          let obj = {};
          obj.id = e.currentTarget.dataset.item.id;
          obj.isBool = false;
          obj.bookId = e.currentTarget.dataset.item.id;
          var queryBean = JSON.stringify(obj);
         wx.navigateTo({ 
            url: '../../pages/fireMapList/fireMapList?queryBean=' + queryBean,
          })
