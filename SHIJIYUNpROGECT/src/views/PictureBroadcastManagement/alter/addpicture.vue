...
<script>
    import {sysAttachmentUpPublicToken,activityImagesBookinsert} from '../../../api/pictureBroadcastManagement/pictureBroadcastApi'
    ...
    export default {
        ...
        methods: {
            ...
            preservation(v){//保存 发布
                ...
                activityImagesBookinsert(data).then(res=>{
            ...
            bannerUrlChange(event,index){//banner图片上传
                let that = this;
//                console.log(that.objData.bannerList.length)
                console.log(index,"bannerList")
                updataImage(event,'TBGLBANNER',that).then(res=>{
//                    console.log(res,"that.objData.bannerList.length")
                    ...
                    that.$set( that.objData.bannerList, that.objData.bannerList[index],res);
                        if((index+1)==that.objData.bannerList.length){
                            if(that.objData.bannerList.length<5){
                                that.objData.bannerList.push({activityImageType:'3',name:'',engName:'',alias:'',imageUrl:'',sort:null,hot:false});
                            }
                        }
                })
            },
            homePageUrlChange(event){ //首页图*上传图片
//                console.log( updataImage(event,'TBGL',this));
//                this.objData.homePageUrl = updataImage(event,'TBGL',this)
...
//                                that.objData.allpictrue.push({imageUrl:ImageUrl,hotChecked:false,sortChecked:false,sort:'0',id:'0'});
                                that.objData.allpictrue.push({activityImageType:'4',name:'',engName:'',alias:'',imageUrl:ImageUrl,sort:0,hot:false,sortChecked:false,id:0});
